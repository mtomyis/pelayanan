<?php 

// $img_logo = file_get_contents('http://localhost/pelayanan/assets/gambar/image001.png'); 
// $img_garis = file_get_contents('http://localhost/pelayanan/assets/gambar/image002.png'); 

// Encode the image string data into base64 
// $url_logos = base64_encode($img_logo);
// $url_gariss = base64_encode($img_garis);
// ini untuk mencoba melihat data image jika dijadikan string base64
// echo $url_logos;
// echo "<br><br>";
// echo $url_gariss;

$url_logo = "data:image/png;base64,";

$url_garis = "data:image/png;base64,R0lGODlhdgIHAHcAMSH+GlNvZnR3YXJlOiBNaWNyb3NvZnQgT2ZmaWNlACH5BAEAAAAALAMAAABvAgYAgAAAAAAAAAJajI+py+0Po5y02ouz3rz7D4biSJbmiabqyrbuC8fyTNf2jef6zvf+DwwKh8Si8YhMKheApvMJjUqn1Kr1is1qt9yu9wsOi8fksvmMTqvX7PZ5CY/L5/S6/VMAADs";
?>

<!DOCTYPE html>
<html>

<body>

<style type="text/css">
	@page{margin: 2cm, 2.5cm, 2cm, 2.5cm}
	#font11{font-size: 11pt;}
	#font12{font-size: 12pt;}
	#font14{font-size: 14pt;}
	#tbl{border-collapse: collapse; border: none; padding: 0; margin: 0; border-spacing: 0;}

</style>
	<!-- <img style="position: absolute; margin-top: 10px; margin-left: 10px;" width=69 height=99 src="<?php echo $url_logo; ?>"> -->

	<table style="width: 100%;" id="tbl">
   	<tr id="font14">
   		<td style="text-align: center;">
		<strong>SURAT KUASA</strong>
   		</td>
   	</tr>
   	</table>
		<br>
	<br>
	<br>

	<table>
		<tr>
			<td>Yang bertanda tangan dibawah ini :</td>
		</tr>
	</table>

	<br>
	<table style="width: 100%" id="tbl">
		<tr>
			<td style="width: 1%">1.</td>
			<td style="width: 10%;">Nama</td>
			<td style="width: 1%;"> : </td>
			<td style="width: 88%;"><?php echo $data_pemberi_kuasa_akta->nama_penduduk ?></td></tr>
		<tr>
			<td></td>
			<td>Tempat/Tanggal Lahir</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->tempat_lahir ?>, <?php echo $data_pemberi_kuasa_akta->tgl_lahir ?></td>
		</tr><tr>
			<td></td>

			<td>Jenis Kelamin</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->jenis_kelamin ?></td>
		</tr><tr>
			<td></td>

			<td>Kewarganegaraan</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->kewarganegaraan ?></td>
		</tr><tr>
			<td></td>

			<td>Agama</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->agama ?></td>
		</tr><tr>
			<td></td>

			<td>Pekerjaan</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->pekerjaan ?></td>
		</tr><tr>
			<td></td>

			<td>Alamat</td>
			<td> : </td>
			<td><?php echo $data_pemberi_kuasa_akta->fullalamat ?></td>
		</tr>
	</table>

	<br>

	<table id="tbl" style="width: 100%"">
		<tr>
			<td style="width: 1%"></td>
			<td style="width: 10%;"><strong><ul>SEBAGAI</ul></strong></td>
			<td style="width: 1%;"> : </td>
			<td style="width: 88%;"><strong><ul>PEMBERI KUASA</ul></strong></td></tr>
		<tr>
		<tr><td></td></tr>
	</table>

	<br>

	<table>
		<tr>
			<td>Yang memberikan kuasanya kepada :</td>
		</tr>
	</table>


	<table style="width: 100%" id="tbl" >
		<tr>
			<td style="width: 1%">2.</td>
			<td style="width: 10%;">Nama</td>
			<td style="width: 1%;"> : </td>
			<td style="width: 88%;"><?php echo $data_penerima_kuasa_akta->nama_penduduk ?></td></tr>
		<tr>
			<td></td>

			<td>Tempat/Tanggal Lahir</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->tempat_lahir ?>, <?php echo $data_penerima_kuasa_akta->tgl_lahir ?></td>
		</tr><tr>
			<td></td>

			<td>Jenis Kelamin</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->jenis_kelamin ?></td>
		</tr><tr>
			<td></td>

			<td>Kewarganegaraan</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->kewarganegaraan ?></td>
		</tr><tr>
			<td></td>

			<td>Agama</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->agama ?></td>
		</tr><tr>
			<td></td>

			<td>Pekerjaan</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->pekerjaan ?></td>
		</tr><tr>
			<td></td>

			<td>Alamat</td>
			<td> : </td>
			<td><?php echo $data_penerima_kuasa_akta->fullalamat ?></td>
		</tr>
	</table>

	<br>

	<table id="tbl" style="width: 100%"">
		<tr>
			<td style="width: 1%"></td>
			<td style="width: 10%;"><strong><ul>SEBAGAI</ul></strong></td>
			<td style="width: 1%;"> : </td>
			<td style="width: 88%;"><strong><ul>PENERIMA KUASA</ul></strong></td></tr>
		<tr>
		<tr><td></td></tr>
	</table>


	<br>
	<table id="tbl">
		<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Surat Kuasa  ini diberikan kepada yang bersangkutan sebagai <?= $data_penerima_kuasa_akta->keterangan_kuasa ?>..</td></tr>
	</table>
	<br>

	<table id="tbl">
		<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Demikian surat keterangan ini dibuat berdasarkan keterangan dari Keluarga yang bersangkutan dan diberikan agar dapat dipergunakan sebagaimana mestinya.</td></tr>
	</table>
	<br>
   			<br>

	<table style="width: 100%;">
   	<tr>
   		<td style="width: 50%; text-align: center;">
   			PENERIMA KUASA
   			<br>
   			<br>
   			<br>
   			
   			<br>
   			<br>
   			<strong><?php echo $data_penerima_kuasa_akta->nama_penduduk ?> </strong>
   		</td>

   		<td style="width: 50%; text-align: center;">
			Bulusari, <?= $data_penerima_kuasa_akta->tanggal ?><br>
			PEMBERI KUASA
   			<br>
   			<br>
   			<br>

   			<br>
   			<strong><?php echo $data_pemberi_kuasa_akta->nama_penduduk ?> </strong>
   		</td>
   	</tr>
   </table>
   <br>
   <br>
   <br>
   <br>
   <table id="tbl" style="text-align: center; width: 100%">
   	<tr>
   		<td style="text-align: center;">Mengetahui</td>
   	</tr>
   	<tr>
   		<td style="text-align: center;">Kepala Desa Bulusari
   		<br>
   	</tr>
   	<tr>
   		
   		<td><br><strong>MUKHLISH, M.Pd.I</strong></td>
   	</tr>
   </table>
	<style type="text/css">
		#tablepengikut {table-layout: auto; border: 1px solid black; border-collapse: collapse; text-align: center; border-spacing: 0;}
	</style>

</body>
</html>

