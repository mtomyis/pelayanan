<?php 

// $img_logo = file_get_contents('http://localhost/pelayanan/assets/gambar/image001.png'); 
// $img_garis = file_get_contents('http://localhost/pelayanan/assets/gambar/image002.png'); 

// Encode the image string data into base64 
// $url_logos = base64_encode($img_logo);
// $url_gariss = base64_encode($img_garis);
// ini untuk mencoba melihat data image jika dijadikan string base64
// echo $url_logos;
// echo "<br><br>";
// echo $url_gariss;

$url_logo = "data:image/png;base64,";

$url_garis = "data:image/png;base64,R0lGODlhdgIHAHcAMSH+GlNvZnR3YXJlOiBNaWNyb3NvZnQgT2ZmaWNlACH5BAEAAAAALAMAAABvAgYAgAAAAAAAAAJajI+py+0Po5y02ouz3rz7D4biSJbmiabqyrbuC8fyTNf2jef6zvf+DwwKh8Si8YhMKheApvMJjUqn1Kr1is1qt9yu9wsOi8fksvmMTqvX7PZ5CY/L5/S6/VMAADs";
?>

<!DOCTYPE html>
<html>

<body>

<style type="text/css">
	@page{margin: 2cm, 2.5cm, 2cm, 2.5cm}
	#font11{font-size: 11pt;}
	#font12{font-size: 12pt;}
	#font14{font-size: 14pt;}
	#tbl{border-collapse: collapse; border: none; padding: 0; margin: 0; border-spacing: 0;}

</style>
	<img style="position: absolute; margin-top: 10px; margin-left: 10px;" width=69 height=99 src="<?php echo $url_logo; ?>">

	<table style="width: 100%;" id="tbl">
   	<tr id="font14">
   		<td style="text-align: center;">
		PEMERINTAH KABUPATEN BANYUWANGI
   		</td>
   	</tr>
   	<tr id="font14">
   		<td style="text-align: center;">
		KECAMATAN KALIPURO
   		</td>
   	</tr>
   	<tr id="font14">
   		<td style="text-align: center;">
		DESA BULUSARI
   		</td>
   	</tr>
   	<tr id="font12">
   		<td style="text-align: center;">
		Jalan Bulusari Nomor 1 Bulusari
   		</td>
   	</tr>
   	<tr id="font12">
   		<td style="text-align: center;">
		Email : ds_bulusari@yahoo.com
   		</td>
   	</tr>
   	</table>

	<table style="width: 100%;">
		<tr>
	   		<td style="text-align: center;"><img width=630 height=7 src="<?php echo $url_garis; ?>"></td>
	   	</tr>
	</table>
		<br>
	<table style="width: 100%;" id="tbl">
		<tr id="font11">
			<td style="text-align: center;">
				<u>SURAT KETERANGAN PINDAH TEMPAT</u></h6>
			</td>
		</tr>
		<tr id="font11">
			<td style="text-align: center;">
				Nomor : 470/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <!-- <?php echo $data_pindah_penduduk->no_surat; ?> --> /429.505.06/2020
			</td>
		</tr>
	</table>

	<br>
	<br>

	<table>
		<tr>
			<td>Kepala Desa Bulusari Kecamatan Kalipuro Kabupaten Banyuwangi menerangkan bahwa :</td>
		</tr>
	</table>

	<br>

	<table style="width: 100%" id="tbl">
		<tr>
			<td style="width: 30%;">Nama</td>
			<td style="width: 1%;"> : </td>
			<td style="width: 69%;"><?php echo $data_pindah_penduduk->nama_penduduk ?></td></tr>
		<tr>
			<td>Tempat/Tanggal Lahir</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->tempat_lahir ?>, <?php echo $data_pindah_penduduk->tgl_lahir ?></td>
		</tr><tr>
			<td>NIK</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->nik ?></td>
		</tr><tr>
			<td>Jenis Kelamin</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->jenis_kelamin ?></td>
		</tr><tr>
			<td>Kewarganegaraan</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->kewarganegaraan ?></td>
		</tr><tr>
			<td>Agama</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->agama ?></td>
		</tr><tr>
			<td>Pekerjaan</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->pekerjaan ?></td>
		</tr><tr>
			<td>Alamat</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->fullalamat ?></td>
		</tr>
	</table>

	<br>

	<table id="tbl">
		<tr><td>Seseorang yang berindititas diatas benar-benar warga desa kami yang sampai saat ini masih berdomisili/bertempat tinggal dialamat  tersebut diatas dan yang bersangkutan akan Pindah Tempat ke</td></tr>
	</table>

	<br>

	<table style="width: 100%" id="tbl">
		<tr>
			<td style="width: 30%">Desa/Kelurahan</td>
			<td style="width: 1%"> : </td>
			<td style="width: 69%"><?php echo $data_pindah_penduduk->desa_baru ?></td>
		</tr><tr>
			<td>Kecamatan</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->kecamatan_baru ?></td>
		</tr><tr>
			<td>Kabupaten</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->kabupaten_baru ?></td>
		</tr><tr>
			<td>Provinsi</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->provinsi_baru ?></td>
		</tr><tr>
			<td>Alasan</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->tujuan ?></td>
		</tr><tr>
			<td>Pada Tanggal</td>
			<td> : </td>
			<td><?php echo $data_pindah_penduduk->tujuan ?></td>
		</tr><tr>
			<td>Pengikut</td>
			<td> : </td>
			<td></td>
		</tr>
	</table>

	<br>
	
	<style type="text/css">
		#tablepengikut {table-layout: auto; border: 1px solid black; border-collapse: collapse; text-align: center; border-spacing: 0;}
	</style>

	<table id="tablepengikut" style="width: 100%;">
      <thead>
      <tr>
        <td id="tablepengikut">No.</td>
        <td id="tablepengikut">NIK</td>
        <td id="tablepengikut">Nama</td>
        <td id="tablepengikut">Tempat, Tgl Lahir</td>
        <td id="tablepengikut">Hubungan Keluarga</td>
        <td id="tablepengikut">Pendidikan</td>
      </tr>
      </thead>
      <tbody>
        <?php
        $no = 1;
        foreach ($data_pindah_penduduk_pengikut as $value) {
        ?>
      <tr>
        <td id="tablepengikut"><?php echo $no++ ?></td>
        <td id="tablepengikut"><?php echo $value->nik_pengikut ?></td> <!-- melihat data lebih lkengkap dengan cara memilih nik -->
        <td id="tablepengikut"><?php echo $value->nama_penduduk ?></td>
        <td id="tablepengikut"><?php echo $value->tempat_lahir ?>, <?php echo $value->tgl_lahir ?></td> 
        <td id="tablepengikut"><?php echo $value->hubungan ?></td>
        <td id="tablepengikut"><?php echo $value->pendidikan ?></td>
      </tr>

    <?php } ?>
    </tbody>
   </table>

   <br>

   <table style="width: 100%;">
   	<tr>
   		<td style="width: 50%;"></td>

   		<td style="width: 50%; text-align: center;">
			Bulusari, 03 Maret 2020
			<br>
			KEPALA DESA  BULUSARI
			<br>
			<br>
			<br>
			<br>
			<br>
			MUKHLISH, M.Pd.I
   		</td>
   	</tr>
   </table>

</body>
</html>

